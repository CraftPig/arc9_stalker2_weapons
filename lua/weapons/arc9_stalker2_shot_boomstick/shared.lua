...
SWEP.WorldModel = "models/weapons/w_smg_p90.mdl" -- w_eq_fraggrenade_thrown, w_knife_t, w_pist_deagle, w_pist_glock18, w_pist_p228, w_pist_usp, w_pist_usp_silencer, w_rif_ak47, w_rif_aug, w_rif_famas, w_rif_galil, w_rif_m4a1, w_rif_m4a1_silencer, w_rif_sg552, w_shot_xm1014, w_smg_mac10, w_smg_mp5, w_smg_p90, w_smg_tmp, w_smg_ump45, w_snip_awp, w_snip_g3sg1, w_snip_scout, w_snip_sg550
...
SWEP.PhysBulletMuzzleVelocity 	= 200 / 0.0254
...
SWEP.Recoil 								= (7) * (GetConVar("arc9_stalker2_mult_recoil"):GetFloat())
...
SWEP.RecoilKickDamping 						= 55.0 -- Camera recoil damping

-- Spread ---------------------------------------------------------------------------------------------
SWEP.Spread 					= (0.2300 / 3) * (GetConVar("arc9_stalker2_mult_spread"):GetFloat())

SWEP.SpreadAddRecoil 			= 0.00758 -- Applied per unit of recoil.
...
SWEP.ActivePos = Vector(-0.5, -0, 1.0) 
...
SWEP.Animations = {
    ...
    ["reload"] = {
		...
		EventTable = {
		    ...
		    {s = "Stalker2.BoomstickSlideBack", t = 11 / 30},
			{s = "Stalker2.BoomstickMagIn", t = 21 / 30},
			{s = "Stalker2.BoomstickSlideForward", t = 38 / 30},
			{s = "Stalker2.BoomstickSlideHit", t = 66 / 30},
        },
    },
	["reload_empty"] = {
        Source = {"reload_empty"},
...
SWEP.Hook_PrimaryAttack = function(self)
	...
	if self:Clip1() == 2 then
		...
		if heatCapacity > 0 then
			...
			local maxChance = 20 -- Maximum chance value
